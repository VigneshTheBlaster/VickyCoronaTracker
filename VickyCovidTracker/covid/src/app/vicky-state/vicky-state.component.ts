...
declare var $:any;
@Component({
  selector: 'app-vicky-state',
  templateUrl: './vicky-state.component.html',
  styleUrls: ['./vicky-state.component.scss']
})
export class VickyStateComponent implements OnInit {
    ...
    var state_name_array = [];
		...
		function addDataToTable(data_array, prev_data_array, isstatus) {
			...
			//add data to table 
				...
				if (diff_confirm > 0) {
					var confirm_pic = confirmed + increases + diff_confirm;
				}
				else if (diff_confirm < 0) {
					confirm_pic = confirmed + decreases + Math.abs(diff_confirm);
				}
				if (diff_deaths > 0) {
					var deaths_pic = deaths + increases + Math.abs(diff_deaths);
				}
				else if (diff_deaths < 0) {
				...
				if (diff_recovered > 0) {
					let increases = "<br><img src=" + upgreen_img_src + " height='12px' width='12px'><span style='font-size:.7rem;'>";
					var recovered_pic = recovered + increases + diff_recovered;
				}
				else if (diff_recovered < 0) {
